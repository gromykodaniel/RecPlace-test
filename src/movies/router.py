...
async def get_details(
    response: Response, id: int, current_user=Depends(get_current_user)
):
    ...
    if not res:
        raise HTTPException(
            status_code=404,
            detail={
                "Error": "not found any movie with this Id. Please cheak another Id!"
            },
        )
    print(current_user)
    return res
...
async def add_to_favorite(
    ...
    if_exist = await FavoritesDAO.find_one_or_none(
        kinopoisk_id=id, user_id=current_user["id"]
    )
    if if_exist:
        raise HTTPException(status_code=404, detail={"Error": "Already at favorites"})
...
async def select_all_from_favorites(
    ...
    if not cur_movies:
        raise HTTPException(status_code=404, detail={"Error": "Nobody at favorites"})
