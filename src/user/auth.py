...
async def verify_mail(code: int, mail: str):
    ...
    if not current or current["code"] == 1:
        raise HTTPException(
            status_code=401, detail="user already veryfied or never exists"
        )
...
async def change_password(code: int, mail: str, new_password):
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            query = (
                update(Users)
                .where(Users.gmail == mail)
                .values(code=1, password=password)
            )
            res = await session.execute(query)
            await session.commit()
            return res.mappings()
...
def create_access_token(data: dict) -> str:
    ...
    jwtt = jwt.encode(to_copy, settings.SECRET_KEY, settings.ALGORITHM)
    return jwtt
